...
import type { NullishPartial } from "../type/record";
 ...
 */
export const hasFalsyProps = <T extends Record<string, unknown>>(
	record: Readonly<T>,
	props: (keyof T)[] = Object.keys(record)
) => props.some((prop) => !record[prop]);
 ...
 */
export const pruneNullishProps = <T extends Record<string | number | symbol, unknown>>(
	obj: Readonly<T>
) => {
